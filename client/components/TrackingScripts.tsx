...
export default function TrackingScripts({ integrations }: TrackingScriptsProps) {
  useEffect(() => {
    // Google Analytics 4
    if (integrations?.googleAnalytics?.measurementId) {
      ...
      const existingGA = document.querySelector(`script[src*="gtag/js?id=${measurementId}"]`);
      if (existingGA) {
        existingGA.remove();
      }
    ...
    // Google Tag Manager
    if (integrations?.googleTagManager?.containerId && integrations.googleTagManager.containerId !== 'GTM-XXXXXXX') {
      ...
      if (!document.querySelector(`iframe[src*="googletagmanager.com/ns.html?id=${containerId}"]`)) {
...
export const trackFormSubmission = (integrations: LandingPageData['integrations'], formData: any) => {
  ...
  if (integrations?.metaPixel?.pixelId && window.fbq) {
    ...
    if (integrations.metaPixel.conversionName) {
      window.fbq('trackCustom', integrations.metaPixel.conversionName, formData);
    }
  }

  // Google Ads conversion
  if (integrations?.googleAds?.conversionId && integrations.googleAds.conversionLabel && window.gtag) {
...
export const submitToFormAPI = async (integrations: LandingPageData['integrations'], formData: any) => {
