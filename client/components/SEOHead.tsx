...
export default function SEOHead({ seo }: SEOHeadProps) {
  useEffect(() => {
    ...
    const title = seo?.title || "Seja um Lojista Oficial Onbongo | Revenda Streetwear Premium";
    const description = seo?.description || "Torne-se um lojista oficial Onbongo e tenha acesso a produtos exclusivos de streetwear. Mais de 30 anos de história, pronta entrega e suporte especializado.";
    const keywords = seo?.keywords || "onbongo, lojista, revenda, streetwear, surfwear, atacado, moda urbana, franquia";
    ...
    const setMetaTag = (name: string, content: string, property = false) => {
      ...
      console.log("✅ Meta tag atualizada:", name, "=", content.substring(0, 50) + "...");
    };
    ...
    setMetaTag("og:description", seo?.openGraph?.description || description, true);
    setMetaTag("og:image", seo?.openGraph?.image || "/images/og-image.jpg", true);
    setMetaTag("og:url", seo?.openGraph?.url || window.location.href, true);
    setMetaTag("og:type", seo?.openGraph?.type || "website", true);
    setMetaTag("og:site_name", seo?.openGraph?.siteName || "Onbongo Lojistas", true);
    ...
    setMetaTag("twitter:image", seo?.twitter?.image || "/images/twitter-card.jpg");
    ...
    if (seo?.structured) {
      ...
      const structuredData = {
        ...
        "logo": seo.structured.organizationLogo || "/images/brand/onbongo-logo.webp",
    ...
    console.log("   - Open Graph tags:", document.querySelectorAll('meta[property^="og:"]').length);
    console.log("   - Twitter tags:", document.querySelectorAll('meta[name^="twitter:"]').length);
